...
window.RPS = (() => {
  ...
  function getPlayerChoice() {
    ...
    if (choice === null) {
      // Проверяем, нажал ли игрок "Отмена"
  ...
  function playRound(playerChoice, computerChoice) {
    ...
    if (playerChoice === computerChoice) {
      ...
      userScore++;
      alert(
        `${language === 'ru' ? 'Вы выиграли!' : 'You won!'} ${resultMessage}`);
    } else {
      computerScore++;
      alert(
        `${language === 'ru' ? 'Вы проиграли!' : 'You lost!'} ${resultMessage}`);
    }
  }
  ...
  return {
    start() {
      const langChoice = prompt(
        'Выберите язык / Choose language (ru/en):'
      ).toLowerCase();
