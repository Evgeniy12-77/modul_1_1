...
const startAdvancedGame = () => {
   let min = parseInt(prompt("Введите минимальное число диапазона:"));
   let max = parseInt(prompt("Введите максимальное число диапазона:"));
   ...
   const attemptsAllowed = Math.ceil((max - min + 1) * 0.3); // 30% от количества чисел в диапазоне
   const userGuesses = [];
   let attempts = 0;

   while (attempts < attemptsAllowed) {
       let guessedNumber = prompt(`Введите ваше число от ${min} до ${max} (осталось попыток: ${attemptsAllowed - attempts}):`);
       ...
       if (attempts === attemptsAllowed) {
           alert(`Вы исчерпали все попытки. Игра окончена. Загаданное число было ${secretNumber}`);
       }
   };
